...
MAX_POSITIONS         = 5
MIN_SCORE             = 65
LOOKBACK_MONTHS       = 12    # 12 months for positional (longer holds need more data)
...
def backtest_symbol(symbol, df):
    ...
    # 52-week high (rolling 252 trading days approx)
    ...
    for i in range(len(df)):
        ...
        if active_trade:
            if row["Low"] <= active_trade["stop_loss"]:
                ...
                trades.append({**active_trade, "exit": active_trade["stop_loss"],
                               "result": "SL", "pnl": round(pnl, 2),
                               "exit_date": df.index[i]})
                active_trade = None
            elif row["High"] >= active_trade["target"]:
                pnl = (active_trade["target"] - active_trade["entry"]) * active_trade["qty"]
                trades.append({**active_trade, "exit": active_trade["target"],
                               "result": "TARGET", "pnl": round(pnl, 2),
                               "exit_date": df.index[i]})
                active_trade = None
...
def run_backtest():
    ...
    print(f"Backtesting positional system on {len(stocks)} stocks | "
